...
- `huwuyou_store_v1`：当前演示状态（schema v3：`settings` / `patients` / `archives` / `needs` / `prices` / `hospitals` / `escorts` 等）。
- `huwuyou_store_corrupt_backup`：解析失败时保存的原始数据备份。
- `huwuyou_patients`：旧版就诊人键，v3 起已并入 `state.patients`，仅保留作备份（不再读写）。

结构升级采用**增量合并**（`CareStore.merge`）：旧数据不会丢失，新增字段与新增表自动补齐，管理员改过的价格与资料保留。
...
上传图片会转成压缩后的 Data URL 存进 LocalStorage（医院封面约 200KB/张、最长边 1280px），容量有限（约 5MB，约 20-25 张）。大量或较大的图片可能触发浏览器存储配额错误，界面会给出提示。

## 自动化回归

```bash
node scripts/smoke-test.js      # 206 项断言：数据层迁移 / CRUD 守卫 / 双向档案 / 复查提醒 / 后台与患者端 UI / 数据流向端到端
node scripts/validate-hospitals.js   # 医院数据校验
```

`smoke-test.js` 使用 DOM 桩在 Node 中按 `index.html` 相同顺序加载全部前端脚本，**零依赖**，可用于每次改动后快速回归。
...
- 首页（**医院实景图片轮播 Hero**（自动播放 / 指示点 / 左右箭头 / 触摸横滑；系统开启减少动态效果时自动停播）+ 信任标签条 + 实时数据条（合作医院/服务项目/在岗陪诊师）+ 服务分类 2×2 + 人工下单/AI 智能下单双入口 + **热门医院图文列表（含医院缩略图）** + 电话咨询条 + 公告）
- 服务二级目录（不展示价格）→ 流程步骤页（统一 3 阶段 7 步：服务前/服务中/服务后，**阶段标题字号加粗放大**）+ **电话咨询卡**（注意事项第 4 条同步显示后台配置的号码）
- 特需服务新增"就诊预约"入口（直达表单并预选服务类型）
- 统一需求表单（人工下单与"我的需求"合并：患者档案可编辑、服务类型可选、**专家预约选项卡**（普通专家/专家门诊/特需专家）、**意向陪诊师下拉（可选，仅登记意向）**、AI 帮填与人工填写实时同步草稿、可后补身份证/检查报告）
- 订单查看（状态筛选 / 详情时间线 / 已接单与未接单状态 / 接单人电话一键拨打）
- 陪诊进度（含接单人信息）
- **我的档案**（就诊记录与复查计划，与平台双向同步：可补充就诊情况/诊疗内容/主诊医生，可标记已复查）
- **复查到期提醒**（首页提醒条 + 通知铃铛；到期前 7 天提醒，逾期高亮）
- **客服中心**（平台咨询电话一键拨号 + 常见问题）
- 个人中心（统计 / 就诊人管理 / 我的需求 / 我的评价）
- 特色 Tab：完整医院目录（搜索 / 分类 / 城市 / 服务量排序筛选 + 实景图卡；**停用医院自动隐藏**）→ 医院详情（含核心优势与"立即预约陪诊"）
- AI 陪诊助手（本地状态机，不使用外部模型；报价话术带出当前咨询电话）
- 通知提醒
- 身份证上传与演示 OCR（统一需求表单内）
- **医院介绍（含医院图片，图文排版）** 与新增医院申请
- 患者评价（已完成订单可评价）
- 紧急联系（规划中）
...
- 需求处理（查看 / 分配陪诊师（**默认高亮患者意向**）/ 推进状态）
- 服务跟踪（已分配 / 服务中订单管理）
- **档案管理**（复查提醒待办 + 就诊档案 CRUD（就诊情况/就诊医院/主诊医生/基本诊疗内容/是否需要复查/复查到期时间）+ 患者数据 CRUD）
- 患者档案（患者数据维护）
- **陪诊师管理（完整 CRUD：新增 / 编辑 / 停用 / 删除，停用后不参与派单）**
- **医院审批与资料（完整 CRUD：新增（含图片上传）/ 编辑 / 停用恢复 / 彻底删除 + 患者申请审批）**
- **服务收费（改价 + 新增服务项（陪诊/专家分组）+ **编辑（改名/描述/单位/分组/价格，改名自动同步未完成需求）** + 停用恢复 + 删除守卫 + 变更记录）**
- **统一二次确认弹窗**（调价、删除患者/档案/医院/服务项/陪诊师、停用医院/服务项/陪诊师；展示影响说明与关键数据，确认前不写入）
- 陪诊报告编辑与发布
- 评价反馈
- **系统设置（咨询电话配置 + 首页轮播配置（自动播放开关、参与医院多选）+ 公告发布 + 数据重置）**
